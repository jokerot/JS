...
const sensors1 = sampleInput.split('\n').map(x => {
...
const pointsOfInterest = allIntersections.flatMap(x=>[[x[0]+1, x[0]], [x[0]-1, x[0]], [x[0], x[0]+1], [x[0], x[0]-1]])
...
const fillLine2 = (x1, y1, x2, y2, pLine, l) => {
    const dX = Math.abs(x1 - x2)
    const dY = Math.abs(y1 - y2)
    const radius = dX + dY
    const cut = radius - (Math.abs(y1 - pLine))
    if (cut > 0) {
        for (let i = 0; i <= cut; i++) {
            l[x1 + i] = '#'
            l[x1 - i] = '#'
...
const sol2 = () => {
    for (let i = 0; i < linePos; i++) {
        const l1 = new Array(size).fill(".")
        const l2 = new Array(size).fill(".")
        sensors1.forEach(e => {
            fillLine2(e[0], e[1], e[2], e[3], linePos + i, l1)
        });
        for (let j = 1; j < size - 1; j++) {
            if ((l1[j - 1] + l1[j] + l1[j + 1]) == "#.#") {
                return ["FOUND IT!!!", j, linePos + i]
            }

        }
        sensors1.forEach(e => {
            fillLine2(e[0], e[1], e[2], e[3], linePos - i, l2)
        });
        for (let j = 1; j < size - 1; j++) {
            if ((l2[j - 1] + l2[j] + l2[j + 1]) == "#.#") {
                return ["FOUND IT!!!", j, linePos - i]
            }

        }
        console.log("Nope", linePos - i);
